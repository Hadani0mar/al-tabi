...
fn build_fast_system_prompt(
    ...
    let pf = product_note
        .split("product_filter=\"")
        .nth(1)
        .and_then(|s| s.split('"').next())
        .filter(|s| !s.is_empty());
    format!(
        "{}{}",
        crate::pattern_catalog::build_executor_system_prompt(erp, pf),
        product_note
    )
}
...
pub async fn handle_with_groq(
    ...
    let memory_block =
        crate::agent_memory::recall_memory_prompt_block(user_text, openai_key, erp).await;
