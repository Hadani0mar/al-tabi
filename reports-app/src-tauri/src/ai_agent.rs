...
pub const DEFAULT_AI_MODEL: &str = "minimax/minimax-m2.7";
...
const PROMPT_SCHEMA_CHAR_LIMIT: usize = 3500;
const HISTORY_MSG_CHAR_LIMIT: usize = 2500;
const RAG_MATCH_COUNT: u32 = 5;
const RAG_DOC_CHAR_LIMIT: usize = 1200;

// ── كاش QUERY_PATTERNS (يُحمَّل مرة واحدة عند أول استخدام) ──────────────────
static QUERY_PATTERNS_CACHE: OnceLock<String> = OnceLock::new();
const QUERY_PATTERNS_EMBED: &str = include_str!("../../QUERY_PATTERNS.md");

/// يبحث عن ملف QUERY_PATTERNS.md في مسارات متعددة بالنسبة لملف التنفيذي
fn load_query_patterns() -> &'static str {
    QUERY_PATTERNS_CACHE.get_or_init(|| {
        // مسارات البحث: نفس مجلد exe، ثم أعلاه مستوى مستوى حتى جذر المشروع
        ...
        let candidates = [
            exe_dir.join("QUERY_PATTERNS.md"),
            exe_dir.join("../QUERY_PATTERNS.md"),
            exe_dir.join("../../QUERY_PATTERNS.md"),
            exe_dir.join("../../../QUERY_PATTERNS.md"),
            exe_dir.join("../../../../QUERY_PATTERNS.md"),
            // مسار ثابت للتطوير (Windows)
            std::path::PathBuf::from(
                r"C:\Users\DELL\Desktop\al-tabi\reports-app\QUERY_PATTERNS.md"
            ),
        ];
        ...
        // احتياطي: ملف مُضمَّن وقت البناء (يعمل في الإنتاج دون نسخ يدوي)
        if QUERY_PATTERNS_EMBED.contains("## PATTERN:") {
            return QUERY_PATTERNS_EMBED.to_string();
        }
        "# QUERY_PATTERNS not found — place QUERY_PATTERNS.md next to the executable.\n".to_string()
    })
}

/// يبحث عن النمط الأنسب بمطابقة الكلمات المفتاحية ويُعيد أقصى قسمين مطابقَين
pub fn search_query_patterns_local(keywords: &str) -> String {
    let content = load_query_patterns();

    // قسّم الملف إلى أقسام عند ## PATTERN:
    let sections: Vec<&str> = content.split("\n## PATTERN:").collect();
    if sections.len() <= 1 {
        return format!("لم يُعثر على ملف QUERY_PATTERNS أو هو فارغ.\n\n{}", content.lines().take(3).collect::<Vec<_>>().join("\n"));
    }
    ...
    let mut scored: Vec<(usize, &str)> = sections.iter().skip(1).map(|section| {
        ...
        let score = kw_words.iter()
            .map(|w| {
                // وزن أعلى للكلمات في سطر TRIGGERS أو اسم PATTERN
...
async fn execute_raw_sql_for_agent(
    ...
    if let Some(block_msg) = crate::agent_tools::antipattern_block_message(sql) {
        return json!({
            "error": block_msg,
...
fn try_handle_greeting(text: &str) -> Option<String> {
    ...
    if greetings.iter().any(|g| t.contains(g)) {
        return Some("👋 أهلاً بك! أنا مساعد قاعدة بيانات Marketing2026.\nاسألني عن:\n📦 المخزون والمنتجات\n💰 فواتير الشراء والبيع\n📅 تواريخ الصلاحية\n👥 العملاء والموردين والديون".to_string());
    }
...
Key patterns: مبيعات يومية موظف, متابعة الديون, ملخص مالي شهري, نواقص نشطة مورد, مقارنة أسعار موردين, رواتب, مصروفات, طلبية شراء ذكية.\n\
...
3. **Is this a complex query?** مبيعات يومية/موظف → `run_query_pattern(\"مبيعات يومية موظف\")`. **ديون + مصاريف + رواتب شهرية** → `run_query_pattern(\"ملخص مالي شهري\")`. **مقارنة أسعار الموردين لمنتج** → `run_query_pattern(\"مقارنة أسعار موردين\", product_filter=\"...\")`. **نواقص نشطة + مورد + آخر سعر شراء** → `run_query_pattern(\"نواقص نشطة مورد\")`. **متابعة نواقص بدون مورد** → `run_query_pattern(\"متابعة النواقص\")`. دراسة منتج → `plan_complex_query` + `execute_query_plan`. طلبية شراء/ديون/صلاحية/جرد/رواتب/تصنيع → `run_query_pattern`. Skip steps 4-5.\n\
                        ...
                        if let Some(tool_calls) = message.get("tool_calls").and_then(|tc| tc.as_array()) {
                            ...
                            for tool_call in filtered_calls {
                                ...
                                if name == "execute_report" {
                                    ...
                                    let schema_data = search_schema(keywords, openai_key).await;
                                    ...
                                    let resp_content = if sql_call_count > MAX_SQL_PER_TURN {
                                        "{\"error\": \"تجاوزت 8 استعلامات في هذا الدور. قدّم إجابة نهائية بناءً على ما حصلت عليه أو اطلب من المستخدم توضيحاً.\"}".to_string()
                                    } else if recent_sql.iter().any(|s| s == &sql_norm) {
                                        "{\"error\": \"هذا الاستعلام تم تنفيذه للتو في هذا الدور بنفس الصيغة. النتيجة لن تتغير. غيّر النهج (table مختلف، WHERE مختلف) أو قدم الإجابة بناءً على ما لديك.\"}".to_string()
                                    } else {
                                    ...
                                    let args: Value = serde_json::from_str(args_str).unwrap_or(empty_json.clone());
                                    let keywords = args.get("keywords").and_then(|k| k.as_str()).unwrap_or("");
                                    let result = search_query_patterns_local(keywords);
                                    let tool_resp_msg = json!({
                                        "role": "tool",
                                        "tool_call_id": tool_call_id,
                                        "content": result
...
async fn execute_db_report(
    ...
    let mut final_sql = report.sql_query.clone();
    final_sql = final_sql.replace("{{DAYS_RECENT}}", "60");
    final_sql = final_sql.replace("{{DAYS_TOTAL}}",  "180");
    final_sql = final_sql.replace("{{TARGET_DATE}}", target_date);

    if !param.is_empty() {
        let condition_str = param.split(',')
            .map(|t| t.trim())
            .filter(|t| !t.is_empty())
            .map(|t| {
                let e = t.replace('\'', "''");
                format!("(I.ITEM_NAME LIKE N'%{}%' OR I.ITEM_MODEL LIKE N'%{}%')", e, e)
            })
            .collect::<Vec<_>>()
            .join(" OR ");
        ...
        final_sql = final_sql.replace("{{SEARCH_CONDITION}}", "1=1");
        final_sql = final_sql.replace("{{SEARCH_TERM}}", "");
        final_sql = final_sql.replace("{{PRODUCTS_LIST}}", "N''");
    }
...
ناد search_schema فقط إذا احتجت جدولاً غير مذكور في DOMAIN_CRITICAL_FACTS.".to_string();
    ...
    let active_pf = crate::agent_tools::extract_product_filter_from_text(user_text).or(session_pf);
...
6. **Domain knowledge — primary tables:**\n\
   ...
   - Customers/suppliers → `dbo.CUSTOMERS` (CUST_ID, CUST_NAME, CUST_VENDOR, CUST_CUSTOMER)\n\
   - Users (employees) → `dbo.USERS` (USERS_ID, FULL_NAME)\n\n\
7. **Output format.** Reply in Arabic with Markdown. Currency: `د.ل`. \
...
2. **Known pattern?** → `run_query_pattern` immediately (آخر يوم مبيعات → \"مبيعات آخر يوم موظف\", ديون/مصاريف → \"ملخص مالي شهري\", مبيعات موظف → \"مبيعات يومية موظف\", مقارنة أسعار موردين → \"مقارنة أسعار موردين\" + product_filter, نواقص+مورد+سعر → \"نواقص نشطة مورد\", متابعة نواقص → \"متابعة النواقص\", ديون → \"متابعة الديون\").\n\
                        ...
                        if let Some(tool_calls) = message.get("tool_calls").and_then(|tc| tc.as_array()) {
                            ...
                            if let Some(content) = message.get("content").and_then(|c| c.as_str()) {
                                    ...
                                    && iter_num < 6
                                {
                                    ...
                                    current_history.push(json!({
                                        "role": "user",
                                        "content": "نفّذ الآن — استدعِ الأداة (run_query_pattern أو execute_raw_sql أو create_excel_report) فوراً. ممنوع الرد بنص فقط دون tool call."
                                    }));
                                    continue;
                                }
                                let final_content =
