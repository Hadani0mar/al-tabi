...
pub fn generate_report_pdf(
    ...
    let n_pages = if row_data.is_empty() {
        1
    } else {
...
fn reshape_arabic(text: &str) -> String {
    ...
    while i < n {
        ...
        if ch == '\u{0644}' && i + 1 < n {
            let mut j = i + 1;
            while j < n && is_transparent(chars[j]) {
                j += 1;
            }
            if j < n {
        ...
        if let Some(forms) = arabic_letter_forms(ch) {
            ...
            let nj =
                is_dual_joining(ch) && next_joinable(&chars, i).map_or(false, |c| right_joins(c));
            let idx = match (pj, nj) {
                (false, false) => 0,
                (true, false) => 1,
                (false, true) => 2,
...
pub fn generate_pos_receipt_pdf(
    ...
    let meta_h = 68.0
        + if meta.address.is_empty() { 0.0 } else { 5.5 }
    ...
    draw_rcpt_table_row(&layer, &font, "البيان", "الكمية", "الإجمالي", y, 8.0, true);
    y -= RCPT_ROW_H;
