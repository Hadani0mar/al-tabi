...
const PRODUCT_COMPREHENSIVE_SQL = `SELECT TOP 30
  ...
  CONVERT(varchar(10), B.UPDATE_DATE, 120) AS [تاريخ تحديث السعر]
FROM dbo.ITEMS I
...
export function GenericReportPage({ connInfo, reportName, reportNameAr }: Props) {
  ...
  useEffect(() => {
    ...
    supabase.from("reports").select("has_parameters").eq("name", reportName).single().then(({ data }) => {
      if (data && data.has_parameters === false) {
        setHasParams(false);
      }
    });
  ...
  async function handleRun() {
    setShowDrop(false);
    if (hasParams && !searchTerm.trim()) {
      setErrorMsg("الرجاء إدخال بيانات للبحث");
      setStatus("error");
      return;
    }
    ...
    try {
      ...
      console.error("[GenericReport]", err);
  ...
  return (
    <div className="p-5 space-y-5 max-w-5xl mx-auto" dir="rtl">
      ...
      <div className="flex items-start gap-2">
        ...
        <Button onClick={handleRun} disabled={status === "loading" || (hasParams && !searchTerm.trim())} className="h-12 px-6 gap-2 shrink-0">
